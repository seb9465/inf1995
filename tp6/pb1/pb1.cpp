...
#pragma region Ecriture et lecture d'un tableau de caractere
    uint8_t tableauEcriture[] = {"E*C*O*L*E* *P*O*L*Y*T*E*C*H*N*I*Q*U*E*"};		//Tableau contenent ce qu'il faut ecrire dans la m�moire externe.
    ...
    uint8_t tableauLecture[sizeTableau];										//Il nous faut preciser la taille de ce tableau pour qu'il aie
																				//la m�me taille que le tableau d'�criture.

	//On passe en parametre l'adresse de d�part, le tableau d'ecriture
	//ainsi que la taille de ce tableau.
    unObjet.ecriture(adresse, tableauEcriture, sizeTableau);					//Fonction permettant l'ecriture dans la m�moire externe.
    ...
    //On parcourt nos deux tableaux afin de les comparer.  D�s que
	//deux donnes ne sont pas les memes, on quitte la boucle immediatement.
    bool donneePareil = true;
    for(uint8_t i = 0; i < sizeTableau && donneePareil; i++) {					//Les deux tableaux sont de m�me tailles.  On les parcours pour les comparer.
